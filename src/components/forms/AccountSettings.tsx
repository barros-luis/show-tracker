...
export function AccountSettings() {
    ...
    const handleUpdateEmail = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast(t('account_settings.email_sent'), "success");
            setNewEmail("");
        } catch (err: any) {
    ...
    const handleUpdatePassword = async (e: React.FormEvent) => {
        ...
        if (newPassword !== confirmPassword) {
            showToast(t('account_settings.password_mismatch'), "error");
            return;
        }
        if (newPassword.length < 6) {
            showToast(t('account_settings.password_length'), "error");
            return;
        }
        ...
        try {
            ...
            if (error) throw error;
            showToast(t('account_settings.password_success'), "success");
            setNewPassword("");
            setConfirmPassword("");
        } catch (err: any) {
            showToast(err.message, "error");
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <Lock className="text-blue-500" /> {t('account_settings.title')}
                </h2>
                <p className="text-gray-500 dark:text-gray-400 mt-1">{t('account_settings.desc')}</p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <Mail size={20} className="text-gray-400" /> {t('account_settings.email_title')}
                </h3>

                <div className="flex flex-col md:flex-row gap-6">
                    <div className="flex-1">
                        <label className="text-xs font-bold text-gray-500 uppercase mb-1.5 block">{t('account_settings.current_email')}</label>
                        ...
                        <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                            <AlertCircle size={12} /> {t('account_settings.email_change_warning')}
                        </p>
                    </div>

                    <form onSubmit={handleUpdateEmail} className="flex-1 space-y-4">
                        <div>
                            <label className="text-xs font-bold text-gray-500 uppercase mb-1.5 block">{t('account_settings.new_email')}</label>
                            <input
                                type="email"
                        ...
                        >
                            {emailLoading ? <Loader2 className="animate-spin" size={16} /> : t('account_settings.update_email')}
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <Lock size={20} className="text-gray-400" /> {t('account_settings.password_title')}
                </h3>

                <form onSubmit={handleUpdatePassword} className="space-y-4 max-w-2xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="text-xs font-bold text-gray-500 uppercase mb-1.5 block">{t('account_settings.new_password')}</label>
                            <input
                                type="password"
                                className="w-full bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-lg py-2.5 px-4 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-colors"
                                placeholder="••••••••"
                        ...
                        <div>
                            <label className="text-xs font-bold text-gray-500 uppercase mb-1.5 block">{t('account_settings.confirm_password')}</label>
                            <input
                                type="password"
                                className="w-full bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-lg py-2.5 px-4 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-colors"
                                placeholder="••••••••"
                    ...
                    <div className="flex justify-end">
                        ...
                        >
                            {passLoading ? <Loader2 className="animate-spin" size={16} /> : t('account_settings.update_password')}
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div className="border border-red-200 dark:border-red-900/30 bg-red-50 dark:bg-red-900/10 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-red-600 dark:text-red-500 mb-2 flex items-center gap-2">
                    <AlertTriangle size={20} /> {t('account_settings.danger_zone')}
                </h3>
                ...
                {!showDeleteConfirm ? (
                    ...
                    >
                        <Trash2 size={16} /> {t('account_settings.delete_account')}
                    </button>
                ) : (
                    <div className="bg-white dark:bg-gray-900 p-4 rounded-xl border border-red-200 dark:border-red-900/50 animate-in fade-in zoom-in-95">
                        <p className="text-sm font-bold text-gray-900 dark:text-white mb-3">
                            {t('account_settings.delete_confirm_text', { code: 'DELETE' })}
                        </p>
                        <div className="flex gap-3">
                            <input
                                ...
                                placeholder={t('account_settings.delete_placeholder')}
                                value={deleteInput}
                            ...
                            >
                                {deleteLoading ? t('account_settings.deleting') : t('account_settings.delete_confirm')}
                            </button>
                            <button
