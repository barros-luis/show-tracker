...
export function StatusDropdown({ currentStatus, onStatusChange, userStatuses }: StatusDropdownProps) {
    ...
    // Find current status from user statuses
    const currentStatusObj = userStatuses.find(s => s.value === currentStatus);
    const displayLabel = currentStatusObj?.label || currentStatus;
    const displayColor = currentStatusObj?.color || 'blue';

    return (
        <div className="relative" ref={dropdownRef}>
            <button
                onClick={() => setIsOpen(!isOpen)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2 cursor-pointer transition-all border ${getStatusColorClasses(displayColor)}`}
            >
                {displayLabel}
                    ...
                    {userStatuses
                        ...
                        .map((status) => (
                            <button
                                ...
                                className={`w-full px-3 py-2 text-left text-xs font-semibold flex items-center gap-2 cursor-pointer transition-colors ${currentStatus === status.value
                                    ? "bg-blue-500/20 text-blue-600 dark:text-white"
                                    : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                                    }`}
                            >
                                {currentStatus === status.value && <Check size={12} />}
